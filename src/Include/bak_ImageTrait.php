...
trait ImageTrait {
	 ...
	 * @public static
	 */
	public static function _parsejpeg(string $file) {
		// check if file exists
		if (!self::safeFileExists($file)) {
			return false;
		}
		...
		$channels = isset($a['channels']) ? (int) $a['channels'] : 3;
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = self::_getUSHORT($data, ($pos - 2)) - 16;
			if ($length < 0) {
				break;
			}
		...
		if (count($icc) > 0) {
			ksort($icc);
			$icc = implode('', $icc);
			// sanity check on profile header
			if (
				ord($icc[36]) !== 0x61 ||
				ord($icc[37]) !== 0x63 ||
				ord($icc[38]) !== 0x73 ||
